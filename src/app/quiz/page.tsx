...
import { ArrowRight, HelpCircle, FileQuestion, Loader2 } from 'lucide-react';
...
export default function QuizPage() {
  ...
  useEffect(() => {
    async function fetchQuizzes() {
      try {
        const data = await firestoreService.getQuizzes();
        setQuizzesList(data);
      } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzesList.length > 0 ? (
            ...
            <div className="col-span-full text-center py-20">
              <p className="text-muted-foreground">No quizzes found in database.</p>
            </div>
          )}
        </div>
      )}
