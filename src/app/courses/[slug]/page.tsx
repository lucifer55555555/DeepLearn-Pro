...
import { Course, LearningTopic } from '@/lib/data';
...
export default function CoursePage({ params }: Props) {
  ...
  useEffect(() => {
    async function fetchCourseData() {
      try {
        const courseData = await firestoreService.getCourse(slug);
        if (courseData) {
          setCourse(courseData);
          const topicData = await firestoreService.getLearningTopic(courseData.topic);
          setTopic(topicData);
        }
      } catch (error) {
  ...
  const handleCompleteCourse = async () => {
    ...
    try {
      ...
      const recommendationInput = {
        ...
        quizPerformance: 'Not yet tracked',
        availableCourses: [], // Will be handled by the AI flow
        availableProjects: [],
      };
