...
export function SubmissionCard({ projectId, projectTitle, solutionCode }: SubmissionCardProps) {
  ...
  const [feedbackResult, setFeedbackResult] = React.useState<ProjectSubmissionFeedbackOutput | null>(null);
  ...
  const handleSubmit = async () => {
    if (!userCode) {
      toast({
        ...
        description: 'Please enter your code to submit.',
      });
      return;
    }
    ...
    try {
      ...
      if (result.isCorrect) {
        if (user) {
          // Call the server-side function to handle the update atomically for logged-in users
          ...
          progressStorage.solveProject(projectId);
          toast({
            title: 'Correct Solution!',
            description: 'Great job! Since you are in Guest Mode, your progress has been saved locally.',
            className: 'bg-green-500/10 border-green-500/20 text-green-500',
          });
        }
      } else {
